    ...
    case "$1" in
        ...
        -H|--hour)
            ...
            if ! [[ "$HOUR" =~ ^[0-9]+$ ]] || [ "$HOUR" -lt 0 ] || [ "$HOUR" -gt 23 ]; then
                log "❌ Ошибка: час должен быть числом от 0 до 23"
                exit 1
            fi
            shift 2
            ;;
        -m|--minute)
            ...
            if ! [[ "$MINUTE" =~ ^[0-9]+$ ]] || [ "$MINUTE" -lt 0 ] || [ "$MINUTE" -gt 59 ]; then
                log "❌ Ошибка: минута должна быть числом от 0 до 59"
                exit 1
            fi
            shift 2
            ;;
...
if [ ! -f "${BACKUP_SCRIPT}" ]; then
    log "❌ Скрипт резервного копирования не найден: ${BACKUP_SCRIPT}"
    exit 1
fi
...
if [ ! -x "${BACKUP_SCRIPT}" ]; then
    ...
    chmod +x "${BACKUP_SCRIPT}" || { log "❌ Не удалось установить права на выполнение"; exit 1; }
fi

# Проверка наличия cron
if ! command -v crontab &> /dev/null; then
    log "❌ crontab не установлен. Установите cron:"
...
if [ ! -d "${LOG_DIR}" ]; then
    ...
    mkdir -p "${LOG_DIR}" || { log "❌ Не удалось создать директорию ${LOG_DIR}"; exit 1; }
fi
