...
cleanup() {
  if [ $? -ne 0 ]; then
    log "❌ Ошибка при восстановлении из резервной копии!"
...
if ! tar -xzf "${BACKUP_FILE}" -C "${TEMP_DIR}"; then
    log "❌ Ошибка при распаковке архива"
    exit 1
fi
...
if [ ! -f "${TEMP_DIR}/${BACKUP_CONTENT}/db_backup.dump" ]; then
    log "❌ Файл дампа базы данных не найден в резервной копии"
    exit 1
fi
...
if ! docker compose cp "${TEMP_DIR}/${BACKUP_CONTENT}/db_backup.dump" postgres:/tmp/; then
    log "❌ Ошибка при копировании дампа в контейнер PostgreSQL"
    exit 1
fi
...
docker compose stop redis || { log "⚠️ Ошибка при остановке сервиса Redis"; }
...
log "🚀 Запуск Redis..."
docker compose start redis || { log "❌ Ошибка при запуске Redis"; exit 1; }
...
# Запуск сервисов
log "🚀 Запуск сервисов..."
docker compose start api grafana || { log "❌ Ошибка при запуске сервисов"; exit 1; }
